...
function generateInvoicePdf(doc, data) {
  ...
  // ロゴ画像に「CONNECT」「コネクト協同組合」の文字が含まれているため、
  // 文字を別途重ねて描画する必要はない（実物の請求書のロゴに合わせる）
  let ry = headerTop;
  const issuerX = left + 290;
  const issuerW = contentW - 290;
  const issuer = data.issuer;
  const logoHeight = 68 * 0.9;
  const logoWidth = logoHeight * (1328 / 416);
  try {
    doc.image(path.join(__dirname, '../public/assets/logo.png'), issuerX, ry - 2, { height: logoHeight });
  } catch (e) {
    // ロゴ画像が読めない場合は省略して続行する
  }
  // 角印（実際に使用している請求書Excel（実物）の座標を実測し、ロゴに対する
  // サイズ・位置の比率を100%再現している：ロゴ高さの約103%のサイズで、
  // ロゴ幅の76%地点あたりの右側に、ほぼロゴ上端に合わせて重ねて配置する）
  try {
    const stampSize = logoHeight * 1.03;
    doc.opacity(0.9).image(
      path.join(__dirname, '../public/assets/stamp.png'),
      issuerX + logoWidth * 0.7623,
      ry - 2 + logoHeight * 0.0203,
  ...
  // ---------- 口座引落日（左）／税額サマリー（右の表） ----------
  // 文字サイズを大きくした分、口座引落日の日付と右の税額サマリー表が
  // 重ならないよう、日付側の幅を実際の表示に必要な分だけに抑えている。
  const dateBoxWidth = 170;
  const summaryTop = y;
  doc.fontSize(11.5).fillColor(INK).text('口座引落日', left, y);
  y += 16;
  doc.fontSize(22);
  boldText(doc, formatJpDate(data.dueDate) || '－', left, y, { width: dateBoxWidth });
  doc.moveTo(left, y + 27).lineTo(left + dateBoxWidth, y + 27).lineWidth(1.4).strokeColor(LINE).stroke();
  y += 33;
  doc.fontSize(9).fillColor(MUTED).text('金融機関が休日の場合は翌営業日になります。', left, y, { width: 240 });

  // 税額サマリー表（右）
  const sumColWidths = [80, 80, 80, 80];
