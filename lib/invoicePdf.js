...
function generateInvoicePdf(doc, data) {
  ...
  // ロゴが大きくなった分、会社名が折り返さないよう実測幅を優先する
  boldText(doc, issuer.name, nameX, ry + 6, { width: Math.max(issuerW - logoWidth - 8, nameWidth + 4) });
  // 角印（実際の請求書の慣例に合わせ、会社名の右端に少し重なる位置に配置する。
  // 下の住所欄に少しかぶっても問題ない、との指示のためサイズを大きめにしている）
  try {
    const stampSize = 60;
