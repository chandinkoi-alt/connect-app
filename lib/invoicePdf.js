...
function generateInvoicePdf(doc, data) {
  ...
  // ロゴ（アイコン＋CONNECTの文字入り画像）を大きく上に、コネクト協同組合の
  // 文字はその下に小さめに表示する（実際の請求書のレイアウトに合わせる）
  let ry = headerTop;
  ...
  const brandY = ry - 2 + logoHeight + 4;
  doc.fontSize(12).fillColor(INK).text(issuer.name, issuerX, brandY, { width: issuerW });
  // 角印（実際の請求書の慣例に合わせ、ロゴの右端に少し重なる位置に配置する。
  // 下の文字欄に少しかぶっても問題ない、との指示のためサイズを大きめにしている）
  try {
    const stampSize = 66;
    doc.opacity(0.9).image(
      path.join(__dirname, '../public/assets/stamp.png'),
      issuerX + logoWidth - 22,
      ry - 2,
