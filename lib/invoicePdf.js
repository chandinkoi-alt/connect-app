...
function generateInvoicePdf(doc, data) {
  ...
  // ---------- タイトル行 ----------
  doc.fontSize(18).fillColor(INK);
  boldText(doc, '口座引落のご案内', left, y, { width: contentW - 190 });

  doc.fontSize(9).fillColor(MUTED);
  doc.text(`発行日　${formatJpDate(data.issueDate)}`, right - 190, y + 4, { width: 190, align: 'right' });
  doc.text(`請求書番号：　${data.invoiceNumber}`, right - 190, y + 17, { width: 190, align: 'right' });
  ...
  doc.fontSize(11).text('御中', left + 190, y + 2);
  y += 22;
  if (company && company.companyNo) {
    doc.fontSize(9).fillColor(MUTED).text(String(company.companyNo).padStart(3, '0'), left, y);
  }
  ...
  // ---------- 口座引落日（左）／税額サマリー（右の表） ----------
  const summaryTop = y;
  doc.fontSize(10).fillColor(INK).text('口座引落日', left, y);
  y += 14;
  doc.fontSize(19);
  boldText(doc, formatJpDate(data.dueDate) || '－', left, y, { width: 220 });
  doc.moveTo(left, y + 24).lineTo(left + 220, y + 24).lineWidth(1.4).strokeColor(LINE).stroke();
  y += 30;
  doc.fontSize(8).fillColor(MUTED).text('金融機関が休日の場合は翌営業日になります。', left, y, { width: 240 });
  ...
  sumColWidths.forEach((w, i) => {
    ...
    doc.fillColor(INK).text(sumLabels[i], cx, sumY + 6, { width: w, align: 'center' });
    cx += w;
  });
  cx = sumX;
  doc.fontSize(9.5);
  ...
  doc.fontSize(9).fillColor(MUTED);
  doc.text(
    `${issuer.bankAccountType}　${issuer.bankAccountMasked}　${issuer.bankAccountNote}`,
    right - 260,
    footerY,
