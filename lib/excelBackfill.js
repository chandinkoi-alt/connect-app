...
const { hostCompanies, workers, sendingOrgs } = require('../db');
...
async function streamExtractRows(buffer) {
  ...
  workbookReader.on('worksheet', (worksheetReader) => {
    if (worksheetReader.name === COMPANY_SHEET) {
      ...
      worksheetReader.on('row', (row) => {
        ...
        sendingOrgMap.set(code, { name, country });
      });
    }
  });
  ...
  return { companyRows, workerRows, sendingOrgMap };
}

async function runBackfill(buffer, summary) {
  logMemory('開始時');
  const { companyRows, workerRows, sendingOrgMap } = await streamExtractRows(buffer);
  logMemory('Excel解析完了');
  ...
  // ---------- 3. 送出機関: インポート時にコード名（TD・HLなど）のまま登録された
  //             団体を、「集計表・リスト」シートの対応表で正式名称・国に補完する。
  //             既に正式名称になっている団体は名前がコードと一致しないため対象外になり、
  //             同じファイルで何度実行しても安全（重複更新にならない）。 ----------
  const existingOrgs = await sendingOrgs.list();
  const orgByCurrentName = new Map(existingOrgs.map((o) => [o.name, o]));
  for (const [code, info] of sendingOrgMap) {
    const existing = orgByCurrentName.get(code);
    if (!existing) continue;
    try {
      await sendingOrgs.update(existing.id, { ...existing, name: info.name, country: info.country });
      summary.sendingOrgsRenamed++;
    } catch (err) {
      summary.errors.push(`送出機関「${code}」の名称補完に失敗: ${err.message}`);
    }
  }
  logMemory('完了時');
}
