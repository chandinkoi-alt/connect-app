...
async function runImport(buffer, summary) {
  ...
  summary.companiesTotal = companyRows.length;

  const companyNoToId = new Map();
  await mapWithConcurrency(companyRows, 6, async (row) => {
    const no = cell(row, 1);
    const name = toText(cell(row, 2));
  ...
  summary.workersTotal = workerRows.length;

  await mapWithConcurrency(workerRows, 6, async (row) => {
    const name = toText(cell(row, 6));
