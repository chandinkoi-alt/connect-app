...
const SENDING_ORG_SHEET = '集計表・リスト';
...
// 「集計表・リスト」シートの左上に、対象者名簿の列K「送出」欄で使われる送出機関の
// 略号（例: TD, HL）と正式名称・国の対応表がある（J列=国, K列=正式名称, L列=略号）。
// 対象者名簿自体には略号しか記録されていないため、この対応表で正式名称を補う。
function extractSendingOrgMapping(row) {
  const country = toText(cell(row, 10));
  const name = toText(cell(row, 11));
  const code = toText(cell(row, 12));
  if (!country || !name || !code) return null;
...
// workbook.xlsx.load()（通常読み込み）はワークブック全体をメモリ上のオブジェクトモデルへ
// 展開するため、この程度のファイル規模でも実測で300MB超のヒープを消費し、メモリの
// 少ない環境（Renderの無料枠など）でOOMの原因になる。ExcelJSの WorkbookReader による
// ストリーミング解析ならその1/5以下で済むが、ふりがな（フリガナ／ルビ）付きの共有文字列
// セルを正しく読めない不具合があるため、事前に xlsxPhoneticFix でそれを取り除いてから
// 解析する（詳細は lib/xlsxPhoneticFix.js を参照）。
async function streamExtract(buffer) {
  ...
  workbookReader.on('worksheet', (worksheetReader) => {
    if (worksheetReader.name === COMPANY_SHEET) {
      ...
      worksheetReader.on('row', (row) => {
        ...
        const data = extractCompanyData(row);
      ...
      worksheetReader.on('row', (row) => {
        ...
        const data = extractWorkerData(row);
        if (data) workerDataList.push(data);
      });
    } else if (worksheetReader.name === WORKER_SHEET_2) {
      worksheetReader.on('row', (row) => {
        if (row.number < 4) return;
        const s = extractSupplementData(row);
        if (s) supplementByPersonalNo.set(s.personalNo, s.data);
      });
    } else if (worksheetReader.name === SENDING_ORG_SHEET) {
      worksheetReader.on('row', (row) => {
        const m = extractSendingOrgMapping(row);
        if (m) sendingOrgMap.set(m.code, m);
      });
    }
    // それ以外のシートはリスナーを付けない(行は内部的に読み飛ばされるだけで保持されない)
  });
  ...
  return { companyDataList, workerDataList, supplementByPersonalNo, sendingOrgMap };
}
...
function logMemory(label) {
  ...
  console.log(
    `[excelImport] ${label}: rss=${(m.rss / 1024 / 1024).toFixed(1)}MB heapUsed=${(m.heapUsed / 1024 / 1024).toFixed(1)}MB external=${(m.external / 1024 / 1024).toFixed(1)}MB`
  );
}

async function runImport(buffer, summary) {
  logMemory('開始時');
  const { companyDataList, workerDataList, supplementByPersonalNo, sendingOrgMap } = await streamExtract(buffer);
  logMemory('Excel解析完了');
  ...
  logMemory('受入企業 書き込み完了');

  // ---------- 2. 送出機関（コード → 実レコード） ----------
  // 対象者名簿には送出機関の略号（例: TD, HL）しか記録されていないため、
  // 「集計表・リスト」シートから読み取った対応表（sendingOrgMap）があればそれで
  // 正式名称・国を登録する。対応表に無い略号は従来通りコードのまま登録し、
  // 後で編集または補完（backfill）できるようにする。
  const sendingOrgIdByKey = new Map();
  const sendingOrgPending = new Map();
  const existingOrgs = await sendingOrgs.list();
  for (const o of existingOrgs) sendingOrgIdByKey.set(o.name, o.id);

  async function resolveSendingOrgId(code) {
    const c = toText(code);
    if (!c || c === '無') return null;
    if (sendingOrgIdByKey.has(c)) return sendingOrgIdByKey.get(c);
    const mapped = sendingOrgMap.get(c);
    if (mapped && sendingOrgIdByKey.has(mapped.name)) return sendingOrgIdByKey.get(mapped.name);
    ...
    const pending = (async () => {
      const created = await sendingOrgs.insert({
        name: mapped ? mapped.name : c,
        country: mapped ? mapped.country : 'ベトナム',
        licenseNumber: '',
        contactPerson: '',
        contactPhone: '',
        contactEmail: '',
        notes: mapped ? '' : 'Excel取り込み（コード名のみ。正式名称は後で編集してください）',
      });
      sendingOrgIdByKey.set(c, created.id);
      if (mapped) sendingOrgIdByKey.set(mapped.name, created.id);
